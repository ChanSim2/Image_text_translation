...
def img_padding_points(min_x, max_x, min_y, max_y, shape, scale=1.0):
    max_h, max_w = shape[0:2]
    scale = scale-1
    max_x += round(min((max_x-min_x),(max_y-min_y))*scale)
    max_y += round(min((max_x-min_x),(max_y-min_y))*scale)
    min_x -= round(min((max_x-min_x),(max_y-min_y))*scale)
    min_y -= round(min((max_x-min_x),(max_y-min_y))*scale)
...
#여기서부터
    ...
    #             image = img_transform(image, rescale=False)
    ...
    #             min_x, max_x, min_y, max_y = img_padding_points(min_x, max_x, min_y, max_y)
    ...
    for i in range(0, realnum+1):
        ...
        n_temp_list = img_padding_points(n_temp_list[0], n_temp_list[1], n_temp_list[2], n_temp_list[3], img_copy.shape, 1.1)
