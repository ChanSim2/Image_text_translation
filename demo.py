...
def demo(opt):
    ...
    with torch.no_grad():
        for image_tensors, image_path_list in demo_loader:
            ...
            dashed_line = '-' * 115
            head = f'{"image_path":25s}\t{"predicted_labels":25s}\t{"translation":25s}\tconfidence score'
            ...
            for img_name, pred, pred_max_prob in zip(image_path_list, preds_str, preds_max_prob):
                ...
                print(f'{img_name:25s}\t{pred:25s}\t{pred_trans:25s}\t{confidence_score:0.4f}')
                log.write(f'{img_name:25s}\t{pred:25s}\t{pred_trans:25s}\t{confidence_score:0.4f}\n')
...
print("time: {}s".format(time.time() - start))  # 현재시각 - 시작시간 = 실행 시간