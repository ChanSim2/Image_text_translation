...
def text_generation(text, shape):
    pygame.init()
    freetype.init()
    font = '/home/MMI22jeongseop/datasets/others/fonts/korean/NanumGothic.ttf'
    font = freetype.Font(font)
    font.antialiased = True
    font.origin = True

    i_t = make_standard_text('/home/MMI22jeongseop/datasets/others/fonts/korean/NanumGothic.ttf', text, shape)
    return i_t

if __name__ == '__main__':
    text = 'hello'
    shape = (64, 256)
    i_t = text_generation(text, shape)
    cv2.imshow('i_t', i_t)
    cv2.waitKey()